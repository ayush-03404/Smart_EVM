...
from config import WS_HOST, WS_PORT, DEFAULT_CANDIDATES
...
class WebSocketServer(QObject):
    ...
    hold_started        = pyqtSignal(int)      # candidate_id — button hold begun
    hold_cancelled      = pyqtSignal(int)      # candidate_id — hold released early
    ...
    def _process(self, raw: str) -> None:
        ...
        if pkt_type == "vote":
            ...
            if isinstance(cid, int) and cid in DEFAULT_CANDIDATES:
                log.info("Vote received → candidate %d", cid)
                self.vote_received.emit(cid)
            else:
                log.debug("Ignored vote for unassigned candidate_id: %s", cid)
            ...
            if isinstance(cid, int) and cid in DEFAULT_CANDIDATES:
                ...
                self.hold_started.emit(cid)
            else:
                log.debug("Ignored hold_start for unassigned candidate_id: %s", cid)

        elif pkt_type == "hold_cancel":
            cid = pkt.get("candidate_id")
            if isinstance(cid, int) and cid in DEFAULT_CANDIDATES:
                log.info("Hold cancelled → candidate %d", cid)
                self.hold_cancelled.emit(cid)
            else:
                log.debug("Ignored hold_cancel for unassigned candidate_id: %s", cid)
