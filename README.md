...
🌐 **Website:** https://smart-evm.lovable.app
...
Download and install from arduino.cc/en/software. Default settings are fine.
...
```
   ...
   ```
Click OK → Tools → Board → Boards Manager
...
Open `SMART_EVM.ino` in Arduino IDE
...
You will get a folder called `Smart_EVM` — keep all files inside it together
---
Step 2 — Run SETUP.bat
> This is the **only step you ever need to do manually**. Everything else is automatic.
Open the `Smart_EVM` folder
...
Double-click `make_exe.bat` (after running SETUP.bat)
Wait 1–2 minutes — PyInstaller compiles the launcher
`launch.exe` appears in the folder — double-click to run with no CMD window
---
Launcher comparison
...
Click the Wi-Fi icon in the Windows taskbar
Find `SMART_EVM` → click Connect
Password: `12345678`
...
4 vote counter cards — one per candidate with live totals
...
Button 1	D1 (GPIO5)	Candidate 1 — Physics
Button 2	D2 (GPIO4)	Candidate 2 — Chemistry
Button 3	D7 (GPIO13)	Candidate 3 — Mathematics
Button 4	D6 (GPIO12)	Candidate 4 — Biology
Button 5	D5 (GPIO14)	(Unused — no candidate assigned)
```
...
ESP8266 NodeMCU
│
├── D1  →  Button 1 → GND   (Candidate 1 — Physics)
├── D2  →  Button 2 → GND   (Candidate 2 — Chemistry)
├── D7  →  Button 3 → GND   (Candidate 3 — Mathematics)
├── D6  →  Button 4 → GND   (Candidate 4 — Biology)
├── D5  →  Button 5 → GND   (Unused — no candidate assigned)
│
...
Check the boot beeps — on power-on the firmware plays two 400 ms beeps as a hardware self-test. If you hear those, the hardware is fine and the issue is elsewhere. If you hear nothing at all, the wiring is wrong.
Swap + and − — some active buzzer modules have reversed markings.
GPIO15 pull-down — if direct wiring still doesn't work, use a transistor as shown in the Wiring Guide above.
Confirm it's an active buzzer — active buzzers work with a plain HIGH/LOW signal. Passive buzzers need a PWM frequency and will not work with this firmware.
...
`launch.exe` will appear in the folder
Double-click it — no CMD window, no setup needed
---
Project File Structure
```
Smart_EVM/
