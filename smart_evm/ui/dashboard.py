...
class VoteCard(_Card):
    ...
    def __init__(self, candidate_id: int, candidate_name: str, parent=None):
        ...
        self._name_lbl.setStyleSheet(f"color: {COLORS['text_muted']}; font-size: 10px; font-weight: 600; letter-spacing: 1px;")
        layout.addWidget(self._name_lbl)

        self._count_lbl = QLabel("0")
        self._count_lbl.setStyleSheet(f"color: {color}; font-size: 36px; font-weight: 700;")
...
class DashboardPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        log_title.setStyleSheet(f"color: {COLORS['text_primary']}; font-weight: 600; font-size: 12px;")
        ...
        self._log_area.setStyleSheet(f"""
            ...
            QScrollBar:vertical {{ background: {COLORS['bg_secondary']}; width: 6px; border-radius: 3px; }}
    ...
    def refresh(self) -> None:
        """Pull latest totals from DB and redraw."""
    ...
    def append_log(self, line: str) -> None:
        """Add a line to the live event feed."""
        lbl = QLabel(line)
        lbl.setWordWrap(True)

        if "Error" in line or "error" in line:
            color = COLORS["accent_red"]
        elif "Vote" in line or "vote" in line:
